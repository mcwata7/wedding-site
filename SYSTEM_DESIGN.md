...
- **Production hosting (GCP e2-micro + Firebase Hosting):** the backend and Postgres run together in Docker Compose (`deploy/compose.prod.yaml`) on a single GCP Always-Free `e2-micro` VM (1GB RAM, 30GB disk, one of `us-west1`/`us-central1`/`us-east1`); a third `proxy` service (Caddy) terminates TLS for the API at a hostname resolved through `sslip.io` from the VM's static external IP (no custom domain owned — see `docs/DECISIONS.md`). Both frontends deploy to Firebase Hosting's free tier (`firebase.json`, two Hosting *sites* — `guest`/`planner` targets) on `*.web.app`/`*.firebaseapp.com` subdomains, which keeps guest-facing static/image traffic off the VM's 1GB/month free egress allowance and off the wedding day critical path if the VM ever has trouble. As with the same-origin Compose setup, the frontends and API are cross-origin here too: `VITE_API_BASE_URL` (read in each frontend's `api/client.ts`, empty in local Compose so that behavior is unchanged) is baked in at Firebase build time to the API's `sslip.io` hostname, and the backend allow-lists all four Firebase origins via `WEDDING_CORS_ALLOWED_ORIGINS` (both `.web.app` and `.firebaseapp.com` per site — Firebase serves each on both, and they're distinct origins). `.github/workflows/deploy.yml` builds the backend image in CI and pushes it to Artifact Registry (never on the VM — a cold Gradle build doesn't fit in 1GB RAM), deploys both frontends to Firebase, then pulls/restarts the VM's compose stack over SSH. CI authenticates to GCP via Workload Identity Federation (no long-lived GCP key in GitHub Secrets), provisioned by `scripts/setup-deploy-auth.sh` — see `docs/DECISIONS.md` for why WIF was chosen over a service-account key, and the one exception (`FIREBASE_SERVICE_ACCOUNT_JSON`, since the Firebase Hosting deploy action has no WIF support yet). `deploy/backup.sh`, run nightly by cron on the VM, dumps Postgres and the media volume to a GCS bucket, replacing what a managed database would otherwise provide. `render.yaml` (repo root) is kept as a dormant fallback Blueprint — see its header comment and `docs/DECISIONS.md` for the full comparison against Render, Railway, and AWS. The root `Makefile` mirrors the three deploy steps (`make deploy-backend` / `deploy-public` / `deploy-internal`, or `make deploy` for all three) for pushing a change from a laptop without waiting on CI — same image tagging and VM SSH/restart logic as `.github/workflows/deploy.yml`, config read from `.env` (see `.env.example`) with `GCP_PROJECT_ID`/`FIREBASE_PROJECT_ID` defaulted from `gcloud config`/`.firebaserc`.
